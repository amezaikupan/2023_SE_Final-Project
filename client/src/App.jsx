...
const App = () => {
  // Ban đầu gán bằng test
  const [conferenceDetailList, setConferenceDetailList] = useState([conferenceDetails]);
  // Gửi lệnh load toàn bộ database lên trang web
  useEffect(() => {
...
const ContentWithFilter = ({ conferenceDetailList }) => {
  // Khai báo state để xử lý hiển thị form filter
  ...
  // Đầu tiến hiện thị toàn bộ dữ liệu
  useEffect(() => {
  ...
  return (
    ...
    <>
      ...
      <Routes>
          ...
          element={
            <>
              <div style = {{minHeight: 500 }}>
                {filteredConferenceDetailList.length > 0 ? (
                  paginatedConferenceDetailList.map((details, index) => (
                    // Truyền dữ liệu từ App xuống ConferenceShortInfo để hiện thị từng đoạn thông tin nhỏ
        ...
        />
        <Route 
          // Truyền dữ liệu từ App xuống ConferenceInfo để chuyển qua của sổ thông tin chi tiết
...
const processConferenceData = (data) => {
  return data.map(item => ({
    ...
    speakers: item.speakers || 'Not available',
